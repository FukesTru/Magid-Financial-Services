      ...
      <ul className="mt-14 grid gap-px overflow-hidden border border-white/8 bg-white/8 sm:grid-cols-2 lg:grid-cols-3">
        {services.map((service, i) => {
          ...
          return (
            ...
            >
              {/*
                The padding moved off the link and onto the body below it, so a
                card photo can run edge to edge. A card with no photo yet is
                laid out exactly as it was before — same padding, same rhythm —
                which is what lets the twelve slots be filled in over time
                without the grid ever looking half-finished.
              */}
              <Link
                href={`/services/${service.slug}`}
                className="group flex h-full flex-col transition-colors duration-300 ease-brand hover:bg-navy-800"
              >
                {photo && <CardPhoto photo={photo} />}
