      ...
      <div className="grid gap-10 lg:grid-cols-3">
        {serviceCategories.map((category) => (
          <div key={category.key}>
            <h3 className="font-sans text-[0.6875rem] font-semibold tracking-[0.18em] text-gold-500 uppercase">
              {category.label}
            </h3>
              ...
              {servicesByCategory(category.key).map((service) => (
                <li key={service.slug}>
                  <Link
                    ...
                    onClick={onNavigate}
                    className="group flex items-start gap-3 rounded-sm px-2 py-2 transition-colors duration-200 hover:bg-white/5"
                  >
                    <ServiceIcon
                      name={service.icon}
                      className="mt-0.5 h-4 w-4 shrink-0 text-gold-500/70 transition-colors duration-200 group-hover:text-gold-400"
                    />
